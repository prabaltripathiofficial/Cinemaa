...
import { useEffect, useState, useMemo, Suspense, JSX } from 'react'; // 1. Import Suspense
...
interface Movie {
  ...
  title: string;
  poster_url: string;
  watch_link?: string; // Make watch_link optional for now
  [key: string]: unknown;
}

// Helper to map genre names to IDs
...
// 2. Move all page logic into a new component
function ResultsPageContent() {
  ...
  if (isLoading) {
    return <div className="text-2xl font-bold">Finding movies...</div>;
  }
  ...
  return (
    <div className="w-full max-w-7xl mx-auto">
      <div className="text-center mb-6">
        <h1 className="text-4xl sm:text-5xl font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-orange-400">{headingText}</h1>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
        ...
        <button onClick={() => router.back()} className="text-gray-300 hover:text-white font-semibold transition-colors cursor-pointer flex-shrink-0">&larr; Refine Search</button>
      </div>
...
export default function ResultsPage() {
  return (
    <main className="bg-gradient-to-b from-gray-900 to-black min-h-screen text-white p-4 sm:p-8 flex items-center justify-center">
      <Suspense fallback={<div className="text-2xl font-bold">Loading results...</div>}>
        <ResultsPageContent />
      </Suspense>
    </main>
  );
